...
- Features grandes: `plan-ceo-review` + `plan-eng-review` (gstack) -> `.agent think` -> `.agent forge` -> `review` + `qa` (gstack) -> `.agent ship` (a `main`).
- Features pequeñas: `.agent forge` -> `review` + `qa --quick` -> `.agent ship` (a `main`).
